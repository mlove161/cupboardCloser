...
#define TEST_UART 2     // Serial1 will be used for the loopback testing with different RX FIFO FULL values
#define RXPIN 3         // GPIO 4 => RX for Serial1
#define TXPIN 1         // GPIO 5 => TX for Serial1
...
void loop() {
  digitalWrite(LED_BUILTIN, LOW);
  uint8_t dataSent[DATA_SIZE];
  for (uint8_t i = 0; i < DATA_SIZE; i++) {
    dataSent[i] = '0' + (i % 10); // fill it with a repeated sequence of 0..9 characters
  }
  Serial.println(dataSent[0]); //Serial.println("Motion detected!");
  if (Serial.available()) {
    char data = Serial.read();
    // Process the received data here
    // Echo back the data to the serial port
    Serial1.write(dataSent, sizeof(dataSent));
  }
  Serial.println("\nYEET!");
}
